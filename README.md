...
## 三、代码提交规范
- 新功能开发、较大改动的bug以及代码重构需要通过开分支提交，并请求其他成员进行代码review以及合并。通过github pull request 提交合并请求。
- 分支说明
...
## 五、ORM生成初始化脚本
- Init ORM: `aerich init -t circleo.settings.TORTOISE_ORM` (Only fisrt, no need)
...
- 小技巧：如果已经使用`aerich migrate`并进行了`aerich upgrade`。可以使用`aerich downgrade`回退版本，然后再进行`aerich migrate`以及`aerich upgrade`
