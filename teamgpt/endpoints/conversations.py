...
async def create_conversations_message(
    ...
    async def async_ask(conversations_input_list, organization_id, conversation_id, title, model, context_number,
                        user):
        ...
        con_org = await ConversationsMessage.filter(user=user_info, conversation_id=conversation_id,
                                                    deleted_at__isnull=True).order_by('-created_at').limit(
            context_number)
        for con in con_org:
        ...
        async def send_gpt():
            message = ''
            new_msg_obj_id = ''
            results = ask(key, message_log[::-1], model, conversation_id)
            async for event in results:
                ...
                if event_data['sta'] == 'run':
                    ...
                    await GptChatMessage.create(in_message=json.dumps(message_log, ensure_ascii=False),
                                                out_message=message,
                                                key=key,
                                                user=user_info,
    ...
    task = loop.create_task(async_ask(conversations_input_list, organization_id, conversation_id, title, model,
                                      context_number, user))
    response = await task
